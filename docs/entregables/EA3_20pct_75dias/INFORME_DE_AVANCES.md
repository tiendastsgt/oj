# INFORME DE AVANCES — Entregable EA3 (20% — Día 75)

**Proyecto:** SGED — Sistema de Gestión de Expedientes Digitales  
**Entregable contractual:** EA3 — UAT (20% del contrato)  
**Hito:** Día 75 desde firma del contrato  
**Versión plantilla:** 1.0.0  
**Estado:** Plantilla — completar con datos del proyecto  

---

## 1. Resumen ejecutivo

Este informe documenta el avance correspondiente al **Entregable EA3 (20%)**, previsto para el **Día 75** respecto a la fecha de firma del contrato. EA3 comprende el plan de pruebas de aceptación de usuario (UAT), la ejecución de UAT con juzgados piloto y el acta de resultados aprobada, así como la evidencia de incidencias detectadas y su resolución, alineado al proyecto SGED (Angular 21 + Java 21 + Oracle 21c).

**Equipo responsable:**
- Amilcar Gil González (Arquitecto)
- Emilio González González (Senior Developer)
- Allan Gil González (Senior BE Developer)
- Alejandro (PM, valida calidad del producto)

---

## 2. Alcance del entregable EA3

| Ítem | Descripción | Referencia |
|------|-------------|------------|
| Plan UAT | Alcance, escenarios, criterios de aceptación y calendario | [EVIDENCIAS.md](./EVIDENCIAS.md#plan-uat) |
| Acta de resultados UAT | Aprobación por juzgados piloto (plantilla) | [EVIDENCIAS.md](./EVIDENCIAS.md#acta-uat) |
| Incidencias y resolución | Registro de incidencias y estado de cierre | [EVIDENCIAS.md](./EVIDENCIAS.md#incidencias) |

---

## 3. Estado por ítem

| Ítem | Estado | Observaciones |
|------|--------|---------------|
| Plan UAT | [x] Aprobado | Guía de pruebas de sistema vinculada |
| Ejecución UAT | [x] Completada | Sprints UX-1 a UX-4 completados |
| Acta de resultados | [ ] En revisión | Pendiente firma de PM y Pilotos |
| Incidencias | [x] Sin incidencias abiertas | 18 hallazgos de auditoría resueltos |

---

## 4. Resumen UAT (completar al cerrar)

| Métrica | Valor |
|---------|--------|
| Escenarios ejecutados | |
| Escenarios aprobados | |
| Incidencias detectadas | |
| Incidencias cerradas | |
| Incidencias abiertas (si aplica) | |
| Juzgados piloto participantes | _Descripción genérica, sin datos sensibles_ |

---

## 5. Desvíos y observaciones

_(Describir aquí cualquier desvío respecto al plan, retrasos o acuerdos con el PM/contratante.)_

---

## 6. Aprobación

| Rol | Nombre | Firma / Fecha |
|-----|--------|----------------|
| PM (calidad) | Alejandro | |
| Representante juzgados piloto (si aplica) | _Genérico_ | |

---

**Documentos de referencia:**
- [Plan detallado](../../general/plan_detallado.md)
- [Roadmap SGED](../../general/ROADMAP_PROYECTO_SGED.md)
- [Auditoría UX](../../auditorias/2026-05-01-auditoria-ux.md)
